...
class MainWindow:
    def __init__(self):
        ...
        self.bottom = StatisticLabel(self.panel_result, 6)
    ...
    def display_bar_charts(self):
        if not self.show_chart:
            ...
            arms = ['Right', 'Left', 'Bottom']
            counts = [self.counter.state.right.count, self.counter.state.left.count, self.counter.state.bottom.count]
            times = [self.counter.state.right.total_time, self.counter.state.left.total_time, self.counter.state.bottom.total_time]

            # Prepare data for table
            table_data = [                
                ['Right', counts[0], formatTime(times[0])],
                ['Left', counts[1], formatTime(times[1])],
                ['Bottom', counts[2], formatTime(times[2])]
            ]
            table_headers = ['Arm', 'Count', 'Time (s)']

            # Create subplots: 2 for bar charts, 1 for the table
            ...
            # Adding the table below the charts
            fig.tight_layout(pad=4.0)
            fig.subplots_adjust(bottom=0.35)  # Adjust bottom space to fit the table

            # Create an additional axis for the table
            ax3 = fig.add_axes([0.1, 0.05, 0.8, 0.2])  # [left, bottom, width, height]
            ax3.xaxis.set_visible(False)
            ax3.yaxis.set_visible(False)
            table = ax3.table(cellText=table_data, colLabels=table_headers, loc='center', cellLoc='center', colColours=['lightgrey']*3)
